...
export function useInsights() {
  ...
  budgets.forEach((budget) => {
    ...
    } else if (percentage > monthProgress + 0.2) {
      insights.push({
        id: `budget-pace-${budget.id}`,
        type: "info",
        title: "Spending Fast",
        message: `You're spending faster than usual. You've used ${Math.round(percentage * 100)}% of your budget, but we're only ${Math.round(monthProgress * 100)}% through the month.`,
      });
    }
  });
